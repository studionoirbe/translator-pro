...
**Standard** is free and covers static translations. **Plus** adds everything AI.
The edition handles are `standard` and `plus`, in that order — Craft reads the
list as least to most feature-rich, and the Plugin Store listing has to use the
same two handles, or installing and switching land on the wrong edition.
...
php craft translingua/license/set-edition standard
...
The toggle is a Plus control, so it only appears on Plus installs — on Standard there
...
Proprietary. The Standard edition may be used free of charge; Plus requires a paid
